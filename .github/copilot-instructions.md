...
- **localStorage keys** are prefixed `ge-analyzer:` (e.g. `ge-analyzer:llm-provider`, `ge-analyzer:view-mode`, `ge-analyzer:top20-sort`, `ge-analyzer:style`, `ge-analyzer:colorway`, `ge-analyzer:deep-history`, `ge-analyzer:compact-tiles`)
...
- **Two-axis theme system (Style × Colorway)**: 4 styles × 6 colorways = 24 combinations via two `<body>` data attributes. **Styles** (`data-style`): Basic, Glassmorphism (`body[data-style="glass"]`), Neumorphism (`body[data-style="neumorphism"]`), Skeuomorphism (`body[data-style="skeuomorphism"]`). **Colorways** (`data-colorway`): Classic Dark (`:root` default), OSRS Brown (`body[data-colorway="osrs"]`), RS3 Modern Blue (`body[data-colorway="rs3-modern"]`), Light (`body[data-colorway="light"]`), Solarized Dark (`body[data-colorway="sol-dark"]`), Solarized Light (`body[data-colorway="sol-light"]`). Each colorway defines helper vars (`--glass-*`, `--neu-*`, `--skeu-*`) consumed by the style structural blocks. New UI elements must use `var(--*)` tokens, never hard-coded colours.
...
| `style.css` | Two-axis theme system (4 styles × 6 colorways via `data-style` + `data-colorway`), cards/tiles/grids, unified analytics modal (details + graph), velocity badges, slider theming, toast notifications, alert inputs, inline alert popovers, data-mgmt buttons, predictive badges, compact-tiles toggle, completed-flips table, CSV export button, provider cost hints, setup guide modal, provider comparison table, settings fieldset groups, badge sizing tokens (`--badge-font-sm/md`, `--badge-padding-sm/md`), `--accent-hype` colour separation (hype vs sell gold), `:focus-visible` keyboard accessibility ring, `.analytics-section-divider` + `.predictive-section` accent border, WCAG AA compliant `--text-muted`, responsive `@media (min-width: 480px)` detail grid + `@media (min-width: 800px)` desktop breakpoint |
...
- **`--accent-hype` separates hype/volume-spike colour from `--accent-gold`**: Hype badges and vol-badges use `var(--accent-hype)`. Sell badges keep `var(--accent-gold)`. Each colorway defines its own `--accent-hype` value.
