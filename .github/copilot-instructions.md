...
- **Payload size guard**: `generateAdvice()` checks JSON body size against `MAX_BODY_BYTES` (95 KB). If exceeded, progressively halves market-data lines (up to 4 attempts) before sending. Groq returns 413 above ~100 KB.
