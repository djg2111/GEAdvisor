...
- **Return on Time (ROT)**: `returnOnTime` field on `RankedItem` — estimated gp/hr = `estFlipProfit × (globalVol / 24) × 0.7`. Replaces the old `fourHourMomentum` field (removed — RS3 guide prices update daily, making sub-daily momentum meaningless). ROT is surfaced in both LLM format strings
- **History write dedup**: `bulkInsert` reads the existing price before writing a history snapshot — skips the insert if the guide price hasn't changed, preventing ~24× daily row bloat from identical hourly polls
- **Database version 3**: `price-history` store uses compound keyPath `[name, timestamp]` (epoch ms). The `day` field is retained for backward compat but is **not** part of the key. Migration from v2 `[name, day]` is automatic — old store is deleted and recreated on version bump
